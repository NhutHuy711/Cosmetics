...
public interface ReviewRepository extends JpaRepository<Review, Integer> {
    ...
    @Query("SELECT r FROM Review r WHERE r.customer.id = ?1 AND ("
            ...
            + "r.productVariant.product.name LIKE %?2%)")
    ...
    @Query("SELECT r FROM Review r WHERE r.productVariant = ?1")
    public Page<Review> findByProduct(ProductVariant productVariant, Pageable pageable);

    @Query("SELECT COUNT(r.id) FROM Review r WHERE r.customer.id = ?1 AND "
            + "r.productVariant.id = ?2")
