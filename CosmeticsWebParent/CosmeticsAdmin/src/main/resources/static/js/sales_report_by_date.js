...
function loadSalesReportByDate(period) {
    const filterParams = buildFilterQueryParams("_date");

    if (period == "custom") {
        startDate = $("#startDate_date").val();
        endDate = $("#endDate_date").val();
        requestURL = contextPath + "reports/sales_by_date/" + startDate + "/" + endDate + filterParams;
    } else {
        requestURL = contextPath + "reports/sales_by_date/" + period + filterParams;
    }
