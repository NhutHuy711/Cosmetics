...
function loadSalesReportByDateForProduct(period) {
    const filterParams = buildFilterQueryParams("_product");

    if (period == "custom") {
        startDate = $("#startDate_product").val();
        endDate = $("#endDate_product").val();
        requestURL = contextPath + "reports/product/" + startDate + "/" + endDate + filterParams;
    } else {
        requestURL = contextPath + "reports/product/" + period + filterParams;
    }
