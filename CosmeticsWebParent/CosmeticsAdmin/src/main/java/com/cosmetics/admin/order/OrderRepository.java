...
public interface OrderRepository extends SearchRepository<Order, Integer> {
    ...
    @Query("""
        SELECT NEW com.cosmetics.common.entity.order.Order(
        ...
        WHERE o.orderTime BETWEEN ?1 AND ?2
            ...
            AND (
                 (o.paymentMethod = com.cosmetics.common.entity.order.PaymentMethod.COD
                      AND o.status = com.cosmetics.common.entity.order.OrderStatus.DELIVERED)
                 OR
                 (o.paymentMethod = com.cosmetics.common.entity.order.PaymentMethod.PAYPAL)
            )
    ...
    WHERE o.orderTime BETWEEN :startTime AND :endTime
      ...
      AND (:payments IS NULL OR o.paymentMethod IN :payments)
    ORDER BY o.orderTime ASC
""")
    List<Order> findByOrderTimeBetweenFiltered(
    ...
    @Query("SELECT COALESCE(SUM(od.quantity), 0) FROM OrderDetail od WHERE od.order.id = :orderId AND od.variant.product.id = :productId")
