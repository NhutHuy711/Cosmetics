...
import com.cosmetics.admin.order.OrderRepository;
import com.cosmetics.admin.storage.ImportRepository;
...
public class MasterOrderReportService extends AbstractReportService {
    ...
    protected List<ReportItem> getReportDataByDateRangeInternal(
            Date startTime, Date endTime, ReportType reportType,
        ...
        List<Order> listOrders = repo.findByOrderTimeBetweenFiltered(startTime, endTime, statuses, payments);
        ...
        for (Order order : listOrders) {
            ...
            ReportItem key = new ReportItem(orderDateString);

            int itemIndex = listReportItems.indexOf(key);
            if (itemIndex >= 0) {
                ReportItem item = listReportItems.get(itemIndex);
